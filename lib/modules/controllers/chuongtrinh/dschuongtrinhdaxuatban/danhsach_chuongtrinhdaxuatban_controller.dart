...
class DanhsachChuongtrinhDaXuatBanController extends GetxController
  ...
  void _updateFilteredList() {
    if (keyWord == null || keyWord!.isEmpty) {
      filteredDsChuongTrinhDaXuatBanData.assignAll(dsChuongTrinhDaXuatBanData);
    } else {
      filteredDsChuongTrinhDaXuatBanData.assignAll(
        dsChuongTrinhDaXuatBanData.where(_itemMatchesSearch),
      );
    }
  }

  void loadDanhSachChuongTrinhDaXuatBan({bool isLoadMore = false}) async {
    if (!isLoadMore) {
      resetController();
    ...
    try {
      ...
      final result = await chuongTrinhProvider.dsChuongTrinhDaXuatBan(
      ...
      if (result.items == null || result.items!.isEmpty) {
        ...
        dsChuongTrinhDaXuatBanData.addAll(result.items!);
  ...
  void loadMore() {
    if (!isLoadingMore.value && hasMoreItems.value) {
      loadDanhSachChuongTrinhDaXuatBan(isLoadMore: true);
    }
  }
  ...
  void resetController() {
    ...
    filteredDsChuongTrinhDaXuatBanData.clear();
    keyWord = "";
    isLoadingMore.value = false;
    currentPage.value = 0;
